...
function isActivePath(pathname: string, href: string): boolean {
  ...
  if (href === "/staff") return pathname === "/staff" || pathname === "/";
  return pathname === href || pathname.startsWith(href + "/");
}

export default function Sidebar({ user, navItems, ctaButton, displayName }: SidebarProps) {
  const pathname = usePathname();
  const router = useRouter();

  const resolvedDisplayName = displayName || user?.full_name || "Caro Giménez";
    ...
    <aside className="sticky top-0 flex h-screen w-[248px] flex-none flex-col border-r border-[#ECE0D0] bg-[#FFFDF9] px-4 py-6">
      ...
      {ctaButton && (
        <button
          ...
          className="mb-3 w-full rounded-xl bg-gradient-to-r from-[#F2937A] to-[#F8C3A8] px-4 py-[10px] text-sm font-bold text-white shadow-sm transition-opacity hover:opacity-90"
        >
          {ctaButton.label}
        </button>
      )}
