...
> **Depends on:** SPEC 01
> **Date:** 2026-08-27
> **Objective:** Implementar las pantallas `ninos.dc.html` y `perfil-nino.dc.html` como `/children` y `/children/profile`, con un estilo visual idéntico al proporcionado.

## Scope

**In:**

- Pantalla `/children` (lista) replicando `ninos.dc.html`: cabecera "GESTIÓN / Niños", botón "Agregar niño" (gradiente), campo de búsqueda con filtrado client-side, divisor "SALA SOLES · 8 niños", grilla de 8 tarjetas.
- Tarjeta de niño: avatar (inicial + color), nombre, "X años · N padres vinculados", badge de alergia (MANÍ/LACTOSA) o "VINCULAR", chevron en los que no tienen badge.
- Pantalla `/children/profile` (perfil estático de Mateo) replicando `perfil-nino.dc.html`: "Volver a Niños", cabecera con avatar/nombre/edad/Editar, bloque "Alergias y notas", tabla (Fecha de nacimiento/Sala/Ingreso), botón "Resumen del día", "PADRES VINCULADOS" con estado ACTIVA/PENDIENTE y "Vincular otro padre".
- Sidebar con estado activo correcto y enlaces reales (Feed → `/`, Niños → `/children`).
...
- Perfil dinámico por niño (`/children/[id]`).
...
2. Modificar `app/components/Sidebar.tsx`: añadir `"use client"`, derivar item activo con `usePathname()`, cablear Feed→`/` y Niños→`/children` (Avisos/Mi cuenta → `#`).
3. Crear `app/components/ChildCard.tsx` (tarjeta de la grilla).
...
6. Crear `app/children/page.tsx`: cabecera + "Agregar niño" + `<ChildrenList />`.
7. Crear `app/children/profile/page.tsx`: perfil estático de Mateo.

## Acceptance criteria

- [ ] `npm run dev` muestra `/children` sin errores en consola.
- [ ] `/children` replica la cabecera "GESTIÓN / Niños", botón "Agregar niño", campo "Buscar niño…" y divisor "SALA SOLES · 8 niños".
...
- [ ] `/children/profile` replica cabecera "Mateo Fernández · 3 años · Sala Soles", bloque de alergias, tabla de datos, "Resumen del día", padres vinculados (Lucía ACTIVA, Diego PENDIENTE) y "Vincular otro padre".
- [ ] "Volver a Niños" navega a `/children`.
...
- **Sí:** rutas en inglés `/children` y `/children/profile`. **No:** `/ninos` ni `[id]` dinámico (sin datos reales por niño).
...
- **Sí:** perfil estático de Mateo. **No:** perfil dinámico por niño.
